...
public class OrderController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateOrder(Guid id, [FromBody] OrderUpdateDto orderUpdateDto)
    {
        try
        {
            ...
            foreach (var item in orderUpdateDto.OrderProducts)
            {
                var product = await _catalogService.GetProductByIdAsync(item.ProductId);
                if (product == null)
                {
            ...
            // Create updated order entity with existing ID
            var updatedOrder = new Domain.Entities.Order(
                id,
            ...
            if (result == null)
            {
                ...
                {
                    Title = "Update Failed",
                    Detail = $"Order with ID {id} could not be updated.",
                    Status = StatusCodes.Status404NotFound
                });
            }
